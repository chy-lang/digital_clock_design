...
/////////////////////////////////////////////////////////////////////////////////////////////////////////// 
//用户配置区
//#define wifi_name "nova 5 Pro"
//#define wifi_password "13792486chen"
//#define protocol "TCP"
//#define IP "www.beijing-time.org"
//#define port "80"
...
const u8* wifista_ssid="nova 5 Pro";			//路由器SSID号
const u8* wifista_encryption="13792486chen";	//wpa/wpa2 aes加密方式
const u8* wifista_password="13792486chen"; 	//连接密码

//WIFI AP模式,模块对外的无线参数,可自行修改.
const u8* wifiap_ssid="ATK-ESP8266";			//对外SSID号
const u8* wifiap_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifiap_password="12345678"; 		//连接密码
const u8* websiteprotocol="TCP";
const u8* websiteIP="www.beijing-time.org";
const u8* port="80";
u8 mystr[30];
u8 mynum;
u8 Time[5];
...
void my_atk_8266_wifista_config(void)
{
	u8 time[9];
  u8 *p;
	u8 i = 15;	
//	p=mymalloc(32);							//申请32字节内存，用于存wifista_ssid，wifista_password
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		atk_8266_quit_trans();//退出透传
		atk_8266_send_cmd("AT+CIPMODE=0","OK",200);  //关闭透传模式	
	...
	delay_ms(100);         //延时100ms等待重启成功
	//设置连接到的WIFI网络名称/加密方式/密码,这几个参数需要根据您自己的路由器设置进行修改!! 
	atk_8266_send_cmd("AT+CIPMUX=0","OK",20);   //0：单连接，1：多连接

	while(atk_8266_send_cmd("AT+CWJAP=\"nova 5 Pro\",\"13792486chen\"","",1000));					//连接目标路由器,并且获得IP
	while(i>0){
		delay_ms(1000);
		i = i-1;
	}
	 while(atk_8266_send_cmd("AT+CIPSTART=\"TCP\",\"www.beijing-time.org\",80","",100));					//连接目标路由器,并且获得IP
	 atk_8266_at_response(0);
	 delay_ms(100);
	 while(atk_8266_send_cmd("AT+CIPMODE=1","OK",20));
	 atk_8266_at_response(0);
	 delay_ms(1000);
   while(atk_8266_send_cmd("AT+CIPSEND","",100));
	 atk_8266_at_response(0);
//	 myfree(p);
//   while(1){
//		 printf("I fucking made it");
//	 }
}

void get_time(void){
	u8 *str1="Date: ";
	u8 *str2="Connection: close";
	u8 i;
	mynum = 0;
	char *strx1=0;
	char *strx2=0;
  ...
  u2_printf("get_time\r\n");
	delay_ms(500);
	atk_8266_at_response(0);
	strx1=strstr((const char*)USART2_RX_BUF,(const char*)str1);
	strx2=strstr((const char*)USART2_RX_BUF,(const char*)str2);
	while(strx1<strx2){
		mystr[mynum]=*strx1;
		strx1++;
		mynum++;
